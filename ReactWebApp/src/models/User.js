...
function getAllUsers(cb){
    ...
    xhr.onload = () => { cb(xhr.status, JSON.parse(xhr.response)); };
    xhr.send();
}
...
export default class User {
    ...
    static getAll(cb){
        getAllUsers((status, res) => {
            if (status === 200){
                cb(res.map((u) => { return new User(u.Username, u.DisplayName, u.Admin)}));
            } else {
                cb(null, status, res);
            }
        });
    ...
    static get(username, cb){
        getUserByUsername(username, (status, res) => {
            if (status === 200){
                cb(new User(res.Username, res.DisplayName, res.Admin));
            } else {
                cb(null, status, res);
            }
        });
